...
GameManager::GameManager(int width, int height, std::string game_title)
	:m_player(std::make_shared<Player>(sf::Vector2f(0,0), m_window)) , m_static_object(), m_enemy() , m_level_num(1),
	m_dead_enemy()
{
	...
	m_strike.setBuffer((ResourceManager::instance().GetAudio(PLAYER_KILL_SOUND)));
	m_player_death.setBuffer(ResourceManager::instance().GetAudio(PLAYER_DEATH_SOUND));
}
...
void GameManager::run()
{
	...
	{
		m_window.create(sf::VideoMode(900,700), "Fun Run"); // setting thw window
		m_sound_track.play();
		load_level(); 
		...
		while (m_window.isOpen())
		{
			...
			if (m_player->GetEatenCandys() == m_map.GetLevelCandys() && m_map.GetLevelCandys() != 0)
			{
			...
			sf::Vector2f position = MakeCenterToView(); // 
			sf::View view(position, sf::Vector2f(m_window.getSize().x / 1.5, m_window.getSize().y / 1.5)); // change to data member
			
			view.zoom(zoomFactor);
			
			m_window.setView(view);
			m_status.draw(m_player, m_window, view.getCenter(), m_map.GetLevelCandys(), m_level_num-1);
			m_window.display();
				...
				case sf::Event::EventType::KeyPressed:
					...
					case sf::Keyboard::Escape: // open pause menu
						...
						if (want_to_play)  //new2
							m_window.create(sf::VideoMode(900,700), "Fun Run");
						break;
					}
					break;
			...
			for (auto& itr : m_enemy)
			{
				if (itr != nullptr)
				{
					...
					CheckCollideEnemy(); // check collide between enemy and the player
				}
			}
			...
			CheckCollideCollectable(); // check if the playercollide with ring and candyss
...
void GameManager::HandleInput()
{
	...
	m_player->SetFallSpeedValue(1.5f); // 
}
...
sf::Vector2f GameManager::MakeCenterToView()
{
	...
	if (pos.y < 315)
		pos.y = 315;
	if (pos.y >(num_rows * 32 - 315))
		pos.y = (num_rows * 32 - 315);
...
// chack collide with candys and rings
