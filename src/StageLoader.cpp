...
// this function gets the player, the enemies, the window, static objets and collectable and fill them
// according the file

void StageLoader::LoadMap(const std::string & namefile, std::shared_ptr <Player>& m_player,
		...
		if (temp.empty())
			...
			for (int j = 0; j < m_stage_x && j < temp.size(); j++)
			{
				...
				case 1:
					...
					break;
				case 4 :
					m_player->SetPlayerPos( sf::Vector2f(j * 3, i *31.5));
					break;
				case 5:
					m_enemy->emplace_back(FactoryEnemy::create("Bee", sf::Vector2f(j * 32, i * 32), window));
					break;
				case 6:
					m_enemy->emplace_back(FactoryEnemy::create("fly", sf::Vector2f(j * 32, i * 32), window));
					break;
				case 7 :
					m_collectable->emplace_back(std::make_unique<Ring>(sf::Vector2f(j * 32, i * 32)));               
					break;
				case 8:
					m_collectable->emplace_back(std::make_unique<Candy>(sf::Vector2f(j * 32, i * 32)));
					m_level_candys++;
					break;
				case 9:
					m_enemy->emplace_back(FactoryEnemy::create("Fairy", sf::Vector2f(j * 32, i * 32), window));
					break;
				default:
...
// this function gets the number of candys e
int StageLoader::GetLevelCandys() const
{
	return m_level_candys;
}
