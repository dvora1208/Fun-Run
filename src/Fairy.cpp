...
bool Fairy::m_registerit = FactoryEnemy::registerit("Fairy",
	[](const sf::Vector2f& pos, sf::RenderWindow & window) -> std::unique_ptr<Enemy> { return std::make_unique<Fairy>(pos, window); });

Fairy::Fairy(sf::Vector2f pos, sf::RenderWindow & window)
	:Enemy(pos, window)
{
	// load the animation picture
	m_walking_animation.emplace_back(std::make_unique<sf::Texture>(std::move(ResourceManager::instance().GetTexture(FAIRY_ANIMATION1))));
	m_walking_animation.emplace_back(std::make_unique<sf::Texture>(std::move(ResourceManager::instance().GetTexture(FAIRY_ANIMATION2))));
	m_walking_animation.emplace_back(std::make_unique<sf::Texture>(std::move(ResourceManager::instance().GetTexture(FAIYR_ANIMATION3))));
...
sf::Vector2f Fairy::get_position() const
{
	return m_position;
}

const sf::Sprite & Fairy::get_pic() const
{
	return m_pic;
}
...
// in charge of preseneting the animation
void Fairy::update(sf::Vector2f centerPos)
{
...
void Fairy::flip_pic_left()
{
	m_pic.setScale(-1.f, 1.f);
}

void Fairy::flip_pic_right()
{
	m_pic.setScale(1.f, 1.f);
}

void Fairy::SetDelta(float time)
{
