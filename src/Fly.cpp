...
bool Fly::m_registerit = FactoryEnemy::registerit("fly",
	[](const sf::Vector2f& pos, sf::RenderWindow & window) -> std::unique_ptr<Enemy> { return std::make_unique<Fly>(pos, window); });

Fly::Fly(sf::Vector2f pos, sf::RenderWindow & window)
	:Enemy(pos, window)
{
	// load the animation texture
	m_walking_animation.emplace_back(std::make_unique<sf::Texture>(std::move(ResourceManager::instance().GetTexture(FLY_ANIMATION1))));
...
sf::Vector2f Fly::get_position() const
{
	return m_position;
}

const sf::Sprite & Fly::get_pic() const
{
	return m_pic;
}
...
void Fly::update(sf::Vector2f centerPos)
{
...
void Fly::flip_pic_left()
{
	m_pic.setScale(1.f, 1.f);
}

void Fly::flip_pic_right()
{
